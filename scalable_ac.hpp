...
class scalable_ac_c {
	...
	bool init(const base_t* symbol_real_frequencies,const max_range_type_t count,max_range_type_t max_symbols,bit_streams::bit_stream_writer_c<writer_type_c>* stream) {
		flush();
		if ((!stream) || (!max_symbols))
			return false;
